...
$(document).ready(function(){
    ...
    $("#pur_submit").click(function () {
        ...
        $.ajax({
            ...
            // dataType: "json",
            success: function () {
                document.getElementById('pur_save').innerHTML = "Purchase number  " + $("#purchase_number").val() + "  details added successfully."
            },
            error: function () {
    ...
    $("#pur_del_btn").click(function () {
        
        var purchase_number = $('#purchase_number').val();
        $.ajax({
            type: "post",
            url: "purchase_details_del",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            data: {
