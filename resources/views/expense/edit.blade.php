                        ...
                        <section class="bg-white dark:bg-gray-900">
                            <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
                                @if ($errors->any())
                                    ...
                                    @endforeach
                                @endif
                                <form action="{{ route('expense.update', $expense) }}" enctype="multipart/form-data"  method="POST" >    
                                    @csrf
                                    @method('patch')
                                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                                        ...
                                        <div class="">
                                            ...
                                            <div class="flex">
                                                <select id="expense_category_id" name="expense_category_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                                    <option value="">Select Expense Category</option>
                                                    @foreach ($expenseCategories as $category)
                                                        <option data-description="{{ $category->description }}" {{ $expense->expense_category_id == $category->id ? 'selected' : '' }}  value="{{ $category->id }}">{{ $category->name }}</option>
                                                    @endforeach
                                                </select>
                                                <x-popover id="expense_description"></x-popover>
                                            </div>
                                        </div>
                                        <div class="">
                                            ...
                                            <select id="project_id" name="project_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                                ...
                                                @foreach ($projects as $project)
                                                    <option {{ $expense->project_id == $project->id ? 'selected' : '' }}  value="{{ $project->id }}">{{ $project->project_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="sm:col-span-2">
                                            <label for="date_of_expense" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Date of Expense</label>
                                            <input type="date" name="date_of_expense" id="date_of_expense" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Date" required="">
                                        </div>
                                        <div class="sm:col-span-2">
                                            <label for="notes" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Notes</label>
                                            <textarea spellcheck="true" id="notes" name="notes" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Leave a comment...">{{ $expense->notes }}</textarea>
                                        </div>
                                        ...
                                        @if ($expense->expenseDocs  != null)
                                            <div class="sm:col-span-2">
                                                <h4 class="block mb-2 text-blue-900 dark:text-white">Uploaded Docs:</h4>
                                                @foreach ($expense->expenseDocs as $doc)
                                                    <p>
                                                        <a class="text-blue-900 dark:text-white" href="{{ asset('storage/'.$doc->doc_name) }}" download>{{ $doc->doc_label }}</a>
                                                    </p>
                                                @endforeach
                                            </div>
                                        @endif
